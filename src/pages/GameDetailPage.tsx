...
function CommentThread({ comment }: { comment: Comment }) {
  return (
    <div className="flex flex-col">
      {/* Main comment */}
      <div className="flex flex-col gap-3 py-4 w-[636px]">
      ...
      <div className="flex flex-col">
        {comment.replies.map((reply, i) => (
          <div key={i} className="flex gap-4 pl-8 py-2 w-[636px]">
            {/* Vertical line */}
...
export function GameDetailPage() {
  return (
    <div className="min-w-[1440px]">
      ...
      {/* ── Comment Section ── */}
      <section className="mx-auto px-[67px] py-6" style={{ maxWidth: 1440 }}>
        {/* Top divider */}
        <div className="border-t border-white/10 mb-6" />

        <div className="flex flex-col items-start">
          {/* Comments heading */}
          <h2 className="font-roboto font-medium text-xl text-white/80 leading-6 w-[636px] mb-0">
            Comments
          </h2>
          ...
          <div className="flex items-center justify-between py-4 w-full">
            <div className="flex items-center">
              {[1, 2, 3, 4, 5].map((n) => (
                <button
                  key={n}
                  className={`w-9 h-9 rounded flex items-center justify-center font-roboto font-medium text-[15px] text-center ${
                    n === 1 ? 'bg-black/80 text-white' : 'text-white/40'
                  }`}
                >
                  {n}
                </button>
